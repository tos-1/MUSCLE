...
class lpt(object):
    ...
    def __init__(self,cosmology='ehu', h=0.7, omega_b=0.0225 , Omega_cdm=0.25, ns =0.96,sigma8=0.8, z_pk=50.,redshift=0.,ng=64 , boxsize=64, 
	sigmaalpt = 4., scheme='zeld', smallscheme = None,makeic=False,return_pos=False,threads=1,extra_info='',seed=1,exact_pk=True):
	...
	if self.makeic==True:
            if not z_pk == redshift:
                raise ValueError("for initial conditions you need z_pk=redshift")

	# for fftw
        self.threads = threads
        cpus = multiprocessing.cpu_count()
        if not cpus>=threads:
            raise ValueError("requested a number of threads > than available cpus")
	...
	# create the folders where binaries are stored
        if (self.makeic == True) or (self.return_pos == False ):
        ...
        if ( (self.makeic == True) and (self.scheme=='2lpt') and (path != None) ):
            # write the param file for Gadget2
            gadgetutils.writeparam( path_sims=path , fileroot=fileroot , scheme=self.scheme,redshift = self.redshift,
                boxsize=self.boxsize,ngrid=self.ng, hubble=self.C.h, ombh2 = self.C.omega_b, Omega0 = self.C.Omega_0 )
            print 'written gadget param file'

	if (self.return_pos==False):
            if self.makeic==False:
		vel = N.zeros_like(pos)
	
            gadgetutils.writegadget(pos,vel,self.redshift,self.boxsize,self.C.Omega_0,1.-self.C.Omega_0,self.C.h , path, fileroot, id=None)
    ...
    def invdiv(self,psi_k):
        ...
        ifftx_obj = pyfftw.FFTW(phixc, phixr, direction='FFTW_BACKWARD',axes=(0,1,2,),threads = self.threads)
        iffty_obj = pyfftw.FFTW(phiyc, phiyr, direction='FFTW_BACKWARD',axes=(0,1,2,),threads = self.threads)
        ifftz_obj = pyfftw.FFTW(phizc, phizr, direction='FFTW_BACKWARD',axes=(0,1,2,),threads = self.threads)


        with warnings.catch_warnings():
            warnings.simplefilter( "ignore" , category = RuntimeWarning )
            #G = -1/self.k**2.
            G = self.cellsize**2./2. /(N.cos(self.kx*self.cellsize) + N.cos(self.ky*self.cellsize)+ N.cos(self.kz*self.cellsize) -3.)
            G.flat[0] = 0
    ...
    def disp_field(self,dk):
        ...
        if self.smallscheme == None:
            ...
            if self.scheme == 'zeld':
                ...
                if self.makeic==True:
                    vel_factor = self.C.Om0z(self.redshift)**(5./9.)*self.C.E(self.redshift)*100./(1.+self.redshift)
                ...
                fft_sc = pyfftw.FFTW(psi_sc, psi_sc_k, direction='FFTW_FORWARD',axes=[0,1,2])
                psi_sc = self.sc(dk)
                ...
                fft_msc = pyfftw.FFTW(psi, psi_msc_k, direction='FFTW_FORWARD',axes=[0,1,2])
                psi = self.muscle(dk)
                ...
                fft_2lpt = pyfftw.FFTW(psi_2lpt, psi_2lpt_k, direction='FFTW_FORWARD',axes=[0,1,2])
    ...
    def sc(self,dk):
        ...
        ifft_za = pyfftw.FFTW(psi_za_k, psi_za, direction='FFTW_BACKWARD',axes=[0,1,2])
    ...
    def muscle(self,dk):
        ...
        twofolds = int(N.log(self.ng)/N.log(2.)) # number of possible iterations
    ...
    def twolpt(self,dk):
        ...
        ifftxx_obj = pyfftw.FFTW(phixxc, phixxr, direction='FFTW_BACKWARD',axes=[0,1,2],threads = self.threads)
        ifftyy_obj = pyfftw.FFTW(phiyyc, phiyyr, direction='FFTW_BACKWARD',axes=[0,1,2],threads = self.threads)
        ifftzz_obj = pyfftw.FFTW(phizzc, phizzr, direction='FFTW_BACKWARD',axes=[0,1,2],threads = self.threads)
        ifftxy_obj = pyfftw.FFTW(phixyc, phixyr, direction='FFTW_BACKWARD',axes=[0,1,2],threads = self.threads)
        ifftxz_obj = pyfftw.FFTW(phixzc, phixzr, direction='FFTW_BACKWARD',axes=[0,1,2],threads = self.threads)
        ifftyz_obj = pyfftw.FFTW(phiyzc, phiyzr, direction='FFTW_BACKWARD',axes=[0,1,2],threads = self.threads)

        with warnings.catch_warnings():
            warnings.simplefilter( "ignore" , category = RuntimeWarning )
            #G = -1/self.k**2.
            G = self.cellsize**2./2. /(N.cos(self.kx*self.cellsize) + N.cos(self.ky*self.cellsize)
                    + N.cos(self.kz*self.cellsize) -3.)
    ...
    def alpt(self,dk):
        ...
        psi_k_alpt[0] = psi_k_small*(1.-gaussian)+psi_k_alpt[0]*gaussian
