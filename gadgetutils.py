...
def writedir(sigmaalpt,extra_info,scheme='2lpt',smallscheme= None,
		redshift=50.,boxsize=256.,ngrid=256, hubble=.7, Omega0 = 0.3, makeic=False):
    '''
    ...
    folder = 'bx' + str(int(boxsize)) + uscr + 'ng'+str(ngrid) + uscr + 'z'+ str(redshift) + uscr +'Om'+'{0:.2f}'.format(Omega0) + '/'
    
    folder_sims = sims + folder

    if smallscheme!=None:
	folder_scheme = folder_sims + smallscheme + '/'

    else:
	folder_scheme = folder_sims + scheme + '/'


    # output folder scheme
    if ((scheme == '2lpt') or (scheme== 'zeld') or (scheme =='muscle') or (scheme=='sc') ):
	fileroot_out = extra_info + 'z'+ str(int(redshift)) + '__0'

    # in alpt save sigmaalpt
    elif (smallscheme == 'sc'):
	fileroot_out = extra_info + 'z'+ str(int(redshift))+'sigmaalpt'+'{0:.1f}'.format(sigmaalpt) + '__0'
    ...
    if makeic == True:
        fileroot_ic =  'IC_'+scheme+ '_z'+ str(int(redshift))
        if os.path.exists( folder_sims + fileroot_ic +'.dat'):
            raise ValueError('these initial conditions already exist')
        else:
...
def writeparam(path_sims, fileroot,scheme='2lpt',redshift=69,boxsize=256.,ngrid=256,
          hubble=.7, ombh2 = 0.045*0.73**2, Omega0 = 0.3, Softening = None, SofteningPhys = None):
    ...
    N.savetxt(path_sims+'zsnaps.txt', redshifts, fmt='%.1f', delimiter=' ', newline='\n', header='', footer='', comments='#', encoding=None)
    N.savetxt(path_sims + fileroot + '.OUT/'+'zsnaps.txt', redshifts, fmt='%.1f', delimiter=' ', newline='\n', header='', footer='', comments='#', encoding=None)

    F.write('OutputListFilename         '+path_sims+fileroot+'.OUT/zsnaps.txt\n')
...
def writegadget(pos,vel,redshift,boxsize,OmegaM,OmegaL,HubbleParam , path_sims, fileroot, id=None):

    pos = N.array(pos)
    vel = N.array(vel)

    pos = N.asfortranarray([pos[0].flatten(), pos[1].flatten(),pos[2].flatten()])
    vel = N.asfortranarray([vel[0].flatten(), vel[1].flatten(),vel[2].flatten()])

    ng = pos.shape[1]

    npartarr = N.array([0,ng,0,0,0,0]).astype(N.uint32)
    # in 10^10 Msolar/h
    mass = N.array([0.,27.7536627*(boxsize**3/float(ng))*OmegaM,0.,0.,0.,0.]).astype(N.float64)
