...
class PowerSpectrum(object):
    ...
    def __init__(self, h=0.7 , omega_b=0.0225 , Omega_cdm=0.25 , ns=0.96 , sigma8 = 0.8):
        """
    ...
    def d1(self,z):
        ...
        """
        return 5*self.Om0z(z)/(1.0 + z)/2.0/(self.Om0z(z)**(4./7.) - self.OmLz(z) + (1.0 + self.Om0z(z)/2.)*(1. + self.OmLz(z)/70.))
    ...
    def table(self,z):
        ...
        """
        import os
        file = 'Pk_at_z'+'{0:.1f}'.format(z) + '_' +'Om'+'{0:.2f}'.format(self.Omega_0) + '.txt'
...
class EisHu(PowerSpectrum):
    ...
    def __init__(self, h=0.7 , omega_b=0.0225 , Omega_cdm=0.25 , ns=0.96 , sigma8 = 0.8):
        ...
        Initialize cosmological parameters
        """
        PowerSpectrum.__init__(self, h=0.7 , omega_b=0.0225 , Omega_cdm=0.25 , ns=0.96 , sigma8 = 0.8)
        self.delH = 1.94e-5*self.Omega_0**(-0.785 - 0.05*N.log(self.Omega_0))*N.exp(-0.95*(self.ns - 1) - 0.169*(self.ns - 1)**2)
    ...
    def T0(self,k):
        ...
        +0.38*N.log(22.3*self.omega_0)* (self.omega_b/self.omega_0)**2.
        
        # Eq. 26
        s = 44.5 * N.log(9.83/self.omega_0)/N.sqrt(1+10*self.omega_b**0.75)

        # Eq. 30
        GammaEffk = self.Omega_0 *self.h * ( alfa_T + (1-alfa_T)/(1+(0.43*k*s*self.h)**4.) )
    ...
    def T(self,k):
       ...
       # Equation 19
        T0t = N.log(N.e + 1.8 * bc * q) / (N.log(N.e + 1.8 * bc * q) + C * q * q)

        # Equation 17
        C1bc = 14.2 + 386.0 / (1.0 + 69.9 * q**1.08)
        T0t1bc = N.log(N.e + 1.8 * bc * q) / (N.log(N.e + 1.8 * bc * q) + C1bc * q * q)
        Tc = f * T0t1bc + (1.0 - f) * T0t

        # Get baryon part of transfer function

        # Equation 24
        bb = 0.5 + ombom0 + (3.0 - 2.0 * ombom0) * N.sqrt((17.2 * om0h2) * (17.2 * om0h2) + 1.0)
    ...
    def pk_ehu(self,k,z):
        ...
        """
        return self.delH**2. * self.Pk_prim(k) * self.T(k)**2 * (self.d1(z)/self.d1(0))**2.
    ...
    def sigma_squared_of_R(self, R=8.):
        ...
        """
        result = quad(self.sigma_squared_integrand,0.,N.Inf, args=(R,),epsrel=1e-16,epsabs=1e-16)[0]
    ...
    def pk_lin(self,k,z):
        ...
        """
        return self.sigma8**2./self.sigma_squared_of_8_raw * self.pk_ehu(k,z)



class PSClass(PowerSpectrum):
    '''Parent class to initialize CLASS instance.
    ...
    def __init__(self, h=0.7 , omega_b=0.0225 , Omega_cdm=0.25 ,ns=0.96, sigma8=0.8,  z_pk=50. ):
        PowerSpectrum.__init__(self,h=h, omega_b=omega_b , Omega_cdm=Omega_cdm , ns=ns , sigma8 = sigma8 )
